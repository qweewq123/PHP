...
Jumei.OrderNew = {
    ...
    initInv : function(){
        ...
        //add by caowei 2015-5-8 11:58:00 @weic1@jumei.com
        var invoiceCheckbox = $('#is_need_invoice');
        if(invoiceCheckbox.is(':disabled')) {
            invInfo.show();
            invInfo.find('#invoice-title').hide();
            invInfo.find('.inv_edit_wrap').hide();
        }
    },
    ...
    enableCartConfirmation : function (){
        ...
        //收货地址
        address_selector.find('.option_box').live('click',function(e){
            Jumei.OrderNew.codCanQrScan(e);
            var ev = e || window.event;
            var input_id = $(this).attr('selector'),
                val = $(this).find('.rdoAddress').first().val(),
                $this = $(this),
                $unvalidateTips = $('#JS_unvalidate_tips');

            if(!$this.find(".id_wrap span").length && is_need_id){ //如果需要身份证号码且没有身份证号
                if(global_authed) {
                    if (!isfirstClickAddress) {
                        //如果不是第一次进入[点击没有身份证的地址],警告关闭后需要显示地址栏
                        Jumei.OrderNew.idCardNeedButNoIdCard(true,$(ev.target).closest('.option_box'));
                    } else {
                        //第一次进入,关闭警告后不需要显示地址栏
                        Jumei.OrderNew.idCardNeedButNoIdCard(false);
                ...
                if (isfirstClickAddress) {
                    ...
                    if (!$(this).hasClass('selected')) {
                        var optionBox = $(this);
                        if (optionBox.attr('ajax_check_address') == undefined) {
                            var addressId = optionBox.find('input[name="address_id"]').val();
                            $.get('/i/account/ajax_check_address', {
                                address_id: addressId
                            }, function (json) {

                                optionBox.attr('ajax_check_address', json.status);
                                if (json.status == 0) {
                                    optionBox.data('address-message', json.message);
                                    // 后来地址有问题的点击触发
                                    $(optionBox).find(".btnEditAddress_new").click()
                                } else {
                                    if ($(optionBox).attr('invalid_structure_code') == 'false') {
                                        selected($(optionBox));
                                    } else {
                                        //之前有问题的点击效果
                                        $(optionBox).find(".btnEditAddress_new").click();
                            ...
                            if ($(optionBox).attr('invalid_structure_code') == 'false' && optionBox.attr('ajax_check_address') == 1) {
                                selected($(optionBox));
                            } else {
                                //之前有问题的点击效果
                                $(optionBox).find(".btnEditAddress_new").click();
            ...
            //如果没有通过身份证认证 则出现顶部提示
            if (is_need_id) {
                if ($this.find('.JS_pass_validate_icon').is(':visible')) {
                    $unvalidateTips.hide();
                } else {
                    $unvalidateTips.show();
                }
            ...
            if (GLOBAL.initData.hasFresh) {
                var address = $this.find('.address_lbl').attr('data_address').split(' ')[0],
                ...
                $.post('/i/confirmation/ajax_check_address_for_fresh', {
                    address: encodeURI(address)
                }, function (data) {
                    if (data.status) {
                        $('#JS_has_fresh_tips').hide();
                        $("#check_pay_form #btn_confirm_pay").removeAttr("disabled").removeClass('disabled');
                    } else {
                        $('#JS_has_fresh_tips').show();
                        $("#check_pay_form #btn_confirm_pay").attr("disabled","true").addClass('disabled');
                    }
                }, 'json')
            }
        });
        ...
        //gateway list
        $('.gateway_ul>li').click(function(e){
            var $this = $(this),
            ...
            if($('.cod_user_balance').length ==0) {
                $(".gateway_COD").append("<span class='cod_user_balance pink'></span>");
            }
            ...
            if ($input.attr('disabled') === false && !$this.hasClass('weihu')) {
                ...
                if ($input.val() === 'COD') {
                    ...
                    var cod_user_balance = $(".cod_user_balance ").show();
                    cod_user_balance.html("亲，货到付款不支持余额和礼品卡支付哦");//货到付款文案显示

                    Jumei.OrderNew.codCanQrScan();

                } else {
                    gift.enableAll();
                    $("#use_balance_checkbox").removeAttr("disabled");//取消选框置灰
                    var allow_balance = Boolean(GLOBAL.initData.allow_balance);
                    if(!allow_balance) {
                        $("#use_balance_checkbox").attr("disabled", true);
                    }
    ...
    idCardNeedButNoIdCard:function(bool,target){
        $.colorbox({
            html: function(){
                return $([
                    "<div class='add_newlight selected_newlight' style='padding-left: 0;width:auto;height:auto;'>",
                    "<div class='cart_pop_tlt'>警告信息</div>",
                    "<div style='text-align: center;'>由于法规要求，跨境购物需要您填写收件人身份证信息</div>",
                    '<div class="site_edit" style="margin:30px 10px 0 10px;text-align: center;"><a href="javascript:" class="submit_btn">确定</a></div>',
                    "</div>"
                ].join('')).find('a.submit_btn').click(function(){
                    $('#cboxClose').click();
                }).end();
            },
            width: 400,
            ...
            onClosed: function () {
                if (bool) {
                    $(target).find(".btnEditAddress_new").click(); //抛出修改地址弹窗事件 并阻止事件向下运行
                }
            }
        });
    },
    ...
    enableCreditTopupLightbox: function(url) {
        ...
        if( $("#check_pay_form").length > 0 ){
            $("#check_pay_form").submit(function(){
                ...
                var giftCard = window.giftCard || {check_payment_select: $.noop};
                if(giftCard.check_payment_select()) {
                    Jumei.Core.lightbox.init({
                        url:  url + '?cache=0',
                        callback: function() {},
                        shadow: true,
    ...
    */
    ...
    codCanQrScan: function (e) {

        var cod_user_balance = $("<span class='cod_user_balance pink'></span>");
        if($('.cod_user_balance').length ==0) {
            $(".gateway_COD").append(cod_user_balance);
        }else {
            cod_user_balance = $('.cod_user_balance');
        }
        var innerHtml = '';
        if(GLOBAL.initData.giftCardInfo) {
            var _sum = GLOBAL.initData.giftCardInfo.giftCardBalance * 1 + GLOBAL.initData.giftCardInfo.balance*1;
            if(_sum > 0) {
                innerHtml = "亲，货到付款不支持余额和礼品卡支付哦";
            }
        }
        cod_user_balance.html(innerHtml);//货到付款文案显示
        var selectedAddress;
        if (e) {
            var optBox = $(e.target).closest('.option_box');
            selectedAddress = optBox.find('.address_lbl').attr('data_address');
        } else {
            selectedAddress = $('.option_box').filter('.selected').find('.address_lbl').attr('data_address') || '';
        }
        var supportCodWithWeixinQrCodeScan = false;
        $.each(GLOBAL.initData.codAlipayQRCodeScope, function (i, value) {
            if(value == 'all') {
                supportCodWithWeixinQrCodeScan = true;
                return false;
            }
            if (selectedAddress.indexOf(value) === 0) {
                supportCodWithWeixinQrCodeScan = true;
                return false;
            }
        });
        if (cod_user_balance.siblings('.supportCodWithWeixinQrCodeScan').length == 0) {
            var appendSpan = $('<span class="supportCodWithWeixinQrCodeScan" style="color:#000;margin-left: 10px;">支持支付宝扫码支付<a href="http://f0.jmstatic.com/static_account/68040.html?v=4" target="_blank" style="color:#0abfde;margin-left: 10px;text-decoration: none;">使用帮助?</a></span>');
            cod_user_balance.after(appendSpan);
        }

        if (supportCodWithWeixinQrCodeScan && cod_user_balance.length > 0) {
            cod_user_balance.siblings('.supportCodWithWeixinQrCodeScan').show();
        } else {
            cod_user_balance.siblings('.supportCodWithWeixinQrCodeScan').hide();
        }

        var isCod = $('.gateway_ul_box input[name="gateway"]:checked').val() === 'COD';
        if (isCod) {
            cod_user_balance.show();
        } else {
            cod_user_balance.hide();
        }
    }
